...
jest.mock('../../functions/twilio-wrappers/conversations.private.js', () => ({
  __esModule: true,
...
jest.mock('../../functions/twilio-wrappers/interactions.private.js', () => ({
  __esModule: true,
  InteractionsOperations: (_, fn) => fn,
}));

describe('invite participant', () => {
  ...
  it.only('createTransferTask is called successfully ', async () => {
    const InviteParticipantTask = require('../../functions/conversation-transfer/invite-participant');
    const handlerFn = InviteParticipantTask.handler;
    JSON.parse = (obj) => obj;
    const mockContext = {
      PATH: 'mockPath',
  ...
  it('createTransferTask gives error due to invalid context ', async () => {
    const CreateTransferTask = require('../../functions/conversation-transfer/invite-participant');
